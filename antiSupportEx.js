		...
		var nowMin = mydate.getMinutes();
			if(localStorage["pRotectCheckMin"] == "" || localStorage["pRotectCheckMin"] == undefined){
				localStorage["pRotectCheckMin"] = mydate.getMinutes();
					notifyMe("莉莉守护盾 2016.9.02.1 运行中",1);
			...
			if (msg.match(/k7001/)) {myMsg += " 加速";criticality += 3;}
			if (msg.match(/k7002/)) {myMsg += " 反加速函数劫持";criticality += 2;}
			...
			if(criticality > 0 ){
				...
				if(criticality < 2){
					criticalityInfo += "中";
				} else if(criticality >= 2 && criticality < 5) {
					criticalityInfo += "高";
				}
			...
			notifyMe("☢Warning☢对:\n" + myMsg + "等ob记录删除后放行"+criticalityInfo+"\n☢Warning☢", 1);
			...
			if (msg.match(/q7001/)) {myMsg += " 加速";criticality += 3;}
			if (msg.match(/q7002/)) {myMsg += " 反加速函数劫持";criticality += 2;}
